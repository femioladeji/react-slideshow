...
import { ButtonClick, IndicatorPropsType, TweenEasingFn } from './types';
import type { FadeProps, ZoomProps, SlideProps } from './types';
...
// export const getUnhandledProps = (ComponentProps: Props, props: Props): any => {
...
export const showPreviousArrow = ({ prevArrow, infinite }: FadeProps | SlideProps | ZoomProps, currentIndex: number, moveSlides: ButtonClick): ReactNode => {
...
export const showNextArrow = (properties: FadeProps | SlideProps | ZoomProps, currentIndex: number, moveSlides: ButtonClick) => {
...
const showDefaultIndicator = (isCurrentPageActive: boolean, key: number, indicatorProps: IndicatorPropsType) => {
  return (
    <li key={key}>
      <button
...
const showCustomIndicator = (isCurrentPageActive: boolean, key: number, indicatorProps: any, eachIndicator: any) => {
...
export const showIndicators = (props: FadeProps | SlideProps | ZoomProps, currentIndex: number, navigate: ButtonClick): ReactNode => {
  ...
  return (
    <ul className="indicators">
      {Array.from({ length: pages }, (_, key) => {
        ...
        const isCurrentPageActive = Math.floor((currentIndex + slidesToScroll - 1) / slidesToScroll) === key;
